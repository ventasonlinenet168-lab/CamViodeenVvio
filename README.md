...
<script type="module">
    ...
    // =========================================================
    // ⚙️ MOTOR DE CÁMARA DIRECTO (AUDIO Y VIDEO)
    // =========================================================
    ...
    window.iniciarCamaraDispositivo = async function() {
        ...
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            alert("⚠️ TU NAVEGADOR BLOQUEÓ LA CÁMARA.\n\nPara transmitir, debes alojar este archivo en un servidor seguro (HTTPS).");
            return;
        }
        ...
        try {
            ...
            if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
                alert("🚫 PERMISO DENEGADO.\n\nEl navegador bloqueó la cámara/micrófono. Sigue estos pasos:\n1. Asegúrate de estar visitando tu enlace HTTPS oficial.\n2. Haz clic en el candado 🔒 junto a la barra de direcciones.\n3. Permite explícitamente el acceso a 'Cámara' y 'Micrófono'.\n4. Recarga la página.");
            } else if (err.name === 'NotFoundError' || err.name === 'DevicesNotFoundError') {
                alert("📱 ERROR DE HARDWARE:\nNo se detectó ninguna cámara o micrófono conectado a este dispositivo.");
            } else if (err.name === 'NotReadableError' || err.name === 'TrackStartError') {
                alert("⚠️ CÁMARA/MICRÓFONO OCUPADO:\nOtra aplicación (como Zoom, WhatsApp, o el sistema operativo) lo está usando en este momento. Ciérrala e intenta de nuevo.");
            } else {
